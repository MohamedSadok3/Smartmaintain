...
// formatDateTime is now imported from utils/dateUtils.js
...
function AlertesPage() {
  ...
  const fetchAlertes = async (page = currentPage) => {
    ...
    try {
      ...
      console.log(`Fetched ${alerts.length} alerts from page ${page}:`, alerts)
      console.log('Pump alerts in response:', alerts.filter(a => a.machine === 'pompe'))
      setRows(alerts)
  ...
  return (
    <section className="space-y-6">
      ...
      <div className="flex flex-wrap gap-3">
        <button
          type="button"
          onClick={() => setFilters({ machine: '', severity: '', status: '', acknowledged: 'false', technician: '', from: '', to: '' })}
        ...
        <button
          type="button"
          onClick={() => setFilters({ machine: '', severity: '', status: 'acknowledged', acknowledged: 'true', technician: '', from: '', to: '' })}
          className={`flex items-center gap-2 rounded-xl px-5 py-3 text-sm font-medium transition-all ${
        ...
        <button
          type="button"
          onClick={() => setFilters({ machine: '', severity: '', status: 'resolved', acknowledged: 'true', technician: '', from: '', to: '' })}
          className={`flex items-center gap-2 rounded-xl px-5 py-3 text-sm font-medium transition-all ${
