  ...
  useEffect(() => {
    ...
    const loadHistoricalAlerts = async () => {
      try {
        ...
        if (mounted && response.data.alerts) {
          const alerts = response.data.alerts
          // Convert alerts to defect history format
          const historyItems = alerts.map((alert) => ({
            timestamp: alert.created_at,
            defect: alert.defect || 'unknown',
            defectScores: {},
            confidence: Math.round((alert.anomaly_score || 0) * 100),
            modelName: 'V7',
            requiredSensors: [],
            status: alert.status,
            alertId: alert.id,
          ...
          setDefectHistory(historyItems)
        }
      } catch (err) {
        // Silently fail - it's ok if historical alerts don't load
        console.debug('Failed to load historical alerts:', err)
      }
    }
