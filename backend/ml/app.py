...
def create_app(ml_service=None):
    ...
    def health():
        ...
        engine_status = ml_service.engine.get_status()
        return {
            "status": "ok",
            "service": "ml",
            "model_version": engine_status.get("model_version", "v7"),
            "supported_models": ml_service.supported_models,
        }
...
def create_and_start_services(app):
    ...
    ml_service = app.extensions["ml_service"]
    logger.info("🚀 Starting ML consumer background thread...")
    consumer_thread = threading.Thread(target=ml_service.consume_sensor_data, daemon=True, name="MLConsumerThread")
    consumer_thread.start()
    logger.info(f"✅ ML consumer thread started: {consumer_thread.name} (daemon={consumer_thread.daemon})")
    return ml_service
