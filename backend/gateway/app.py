...
def authenticate_socket(auth):
    ...
    if payload.get("role") == "superadmin":
        ...
        logger.info("WebSocket client joined room: %s", SUPERADMIN_ROOM)
        return True
    ...
    if plant_id is None:
        ...
        return False
    room = f"plant:{int(plant_id)}"
    join_room(room)
    logger.info("WebSocket client joined room: %s (user=%s)", room, payload.get("email", "?"))
    return True
