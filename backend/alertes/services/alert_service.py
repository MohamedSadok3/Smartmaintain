...
from shared.timezone_utils import now_local
...
class AlertService:
    ...
    def get_alert_with_users(self, alert_id):
        ...
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                ...
                if not alert:
                    logger.error(f"Failed to deserialize alert {alert_id} from database row")
                    return None
                return alert
    ...
    def list_alerts(self, current_user, filters=None):
        ...
        if current_role == ROLE_TECHNICIEN:
            # Technicians see alerts assigned to them OR unassigned alerts
            where_parts.append(sql.SQL("(a.assigned_to = %s OR a.assigned_to IS NULL)"))
    ...
    def _apply_alert_update(self, cur, alert_id, fields, values):
        ...
        if not fetched_row:
            raise AlertNotFoundException("Alerte introuvable après mise à jour.")
        alert = Alert.from_row(fetched_row)
        if not alert:
            raise AlertNotFoundException("Impossible de désérialiser l'alerte.")
        return alert
        ...
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                try:
                    ...
                    conn.rollback()
                    logger.error(f"Failed to assign alert {alert_id}: {e}")
                    raise
